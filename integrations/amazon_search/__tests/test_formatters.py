...
def test_should_mark_the_product_as_sponsored_when_it_is(products: List[dict]):
    target_product = next(p for p in products if p["asin"] == "B0BJZKYL1D")
    assert target_product["is_sponsored"]


def test_should_not_mark_the_product_as_sponsored_when_it_is_not(products: List[dict]):
    target_product = next(p for p in products if p["asin"] == "B0BGBHXSWH")
    assert not target_product["is_sponsored"]


def test_should_not_filter_out_products_that_have_no_reviews_data(products: List[dict]):
    target_product = next(p for p in products if p["asin"] == "B0CMT4P1K8")
    assert "reviews" not in target_product


def test_should_filter_out_products_that_have_no_pricing_data(products: List[dict]):
    assert all(p["price_usd"] for p in products)


def test_should_remove_duplicated_products_and_keep_the_first_one(products: List[dict]):
